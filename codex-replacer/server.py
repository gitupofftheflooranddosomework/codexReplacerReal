...
CONVERSATION_CONTINUITY_INSTRUCTIONS = (
    ...
    "Execution boundary: the user console/control VM is reserved exclusively for Mark and is never a bot work machine. This MCP endpoint is a broker. For interactive repository, shell, or browser work first use vm_lab_acquire with your stable bot name, then vm_lab_exec or vm_browser_* with that lease and always vm_lab_release when finished. For transferable CPU-heavy or long-running builds, test suites, compilers, recursive validators/scans, Docker builds, package builds, PDF/link validation, and similar work, use vm_job_submit. When two or more independent heavy tasks are available, prefer vm_job_submit_batch so one MCP call can fill all six shared workers immediately. The homeserver scheduler chooses workers automatically and runs up to six independent heavy jobs concurrently. Poll with vm_job_status and never resubmit a running job just to check it. Controller-local host/fs/git/docker/browser tools are intentionally unavailable to bots. "
